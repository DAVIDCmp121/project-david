...
export default function AdminProducts() {
  ...
  async function addProduct() {
    if (!form.name || !form.price) {
      alert('ກະລນາໃສ່ຊືສນຄ້າ ແລະ ລາຄາ');
      return;
    }
  ...
  async function deleteProduct(id) {
    if (!window.confirm('ຕອງການລຶບສນຄ້ານບ?')) return;
  ...
  function askSaveEdit() {
    if (!editForm.name || !editForm.price) {
      alert('ກະລນາໃສ່ຊື່ສນຄາ ແລະ ລາຄາ');
      return;
    }
    ...
    <div>
      ...
      {editingProduct && (
        <div className="modal-overlay" onClick={(e) => { if (e.target === e.currentTarget) closeEditModal(); }}>
          <div className="modal-box" style={{ background: '#fff', color: '#1f2937' }}>
            <button className="modal-close" style={{ color: '#1f2937' }} onClick={closeEditModal}>✕</button>
            <h2 style={{ color: 'var(--navy)' }}>ແກ້ໄຂສິນຄ້າ</h2>
            {editingProduct.image && (
              <img src={editingProduct.image} width={60} height={60} style={{ objectFit: 'cover', borderRadius: 6, marginBottom: 8 }} alt="" />
            )}
            <input placeholder="ຊື່ສິນຄ້າ" value={editForm.name} onChange={(e) => setEditForm({ ...editForm, name: e.target.value })} />
            <input placeholder="ລາຄາ" type="number" value={editForm.price} onChange={(e) => setEditForm({ ...editForm, price: e.target.value })} />
            <input placeholder="ໄຊສ໌" value={editForm.size} onChange={(e) => setEditForm({ ...editForm, size: e.target.value })} />
            <input placeholder="ສີ" value={editForm.color} onChange={(e) => setEditForm({ ...editForm, color: e.target.value })} />
            <input placeholder="ຈຳນວນສະຕັອກ" type="number" value={editForm.stock} onChange={(e) => setEditForm({ ...editForm, stock: e.target.value })} />
            <input type="file" accept="image/*" ref={editImageRef} />
            <div style={{ display: 'flex', gap: 8, marginTop: 10 }}>
              <button className="primary" onClick={askSaveEdit}>ບັນທຶກ</button>
              <button onClick={closeEditModal}>ຍົກເລີກ</button>
            </div>
          </div>
        </div>
      )}
      ...
      {confirmSaveOpen && (
        <div className="modal-overlay" onClick={(e) => { if (e.target === e.currentTarget) setConfirmSaveOpen(false); }}>
          <div className="modal-box" style={{ background: '#fff', color: '#1f2937', maxWidth: 340, textAlign: 'center' }}>
            ...
            <div style={{ display: 'flex', gap: 10, justifyContent: 'center' }}>
              ...
              <button onClick={() => setConfirmSaveOpen(false)}>ຍົກເລີກ</button>
            </div>
          </div>
        </div>
      )}
